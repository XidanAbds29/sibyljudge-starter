  ...
  >
    {[0, 1, 2].map((i) => (
      <motion.div
        ...
        variants={{
          initial: { y: 0 },
          animate: { y: [0, -5, 0], transition: { duration: 0.8, repeat: Infinity, ease: "easeInOut" } },
        }}
      />
    ))}
  </motion.div>
...
const AuthForm = ({ isLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    setLoading(true);
    setLoadingMessage(isLogin ? "Verifying Credentials..." : "Initiating Secure Signup...");
    ...
    try {
      ...
      if (isLogin) {
        ...
        response = await signUp(email, password, username, { institution, bio });
      }
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          >
            ...
            <p className="text-xl font-semibold text-cyan-400 mb-2 tracking-wider">{loadingMessage}</p>
            <LoadingDots />
          ...
          <motion.div
            ...
            transition={{ duration: 0.3 }}
          >
            <h2 className="text-3xl font-bold text-center text-cyan-400" style={{ textShadow: '0 0 8px rgba(0, 255, 255, 0.5)'}}>
              {isLogin ? 'Access Terminal' : 'Register Operative'}
            </h2>
            <form onSubmit={handleSubmit} className="mt-8 space-y-6">
              {!isLogin && (
                <div>
                  <label htmlFor="username" className="block text-sm font-medium text-gray-300">Callsign (Username)</label>
              ...
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-gray-300">Email Matrix ID</label>
                <input id="email" name="email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)}
                  className="mt-1 block w-full px-3 py-2.5 bg-gray-800/70 border border-gray-700 rounded-md shadow-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-gray-200 transition-colors"
                />
              </div>
              <div>
                <label htmlFor="password" className="block text-sm font-medium text-gray-300">Access Code (Password)</label>
              ...
              {!isLogin && (
                <>
                  <div>
                    <label htmlFor="institution" className="block text-sm font-medium text-gray-300">Affiliation (Institution - Optional)</label>
                    <input id="institution" name="institution" type="text" value={institution} onChange={(e) => setInstitution(e.target.value)}
                      className="mt-1 block w-full px-3 py-2.5 bg-gray-800/70 border border-gray-700 rounded-md shadow-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-gray-200 transition-colors"
                    />
                  </div>
                  <div>
                    <label htmlFor="bio" className="block text-sm font-medium text-gray-300">Profile Dossier (Bio - Optional)</label>
              ...
              {error && <p className="text-sm text-red-400 bg-red-900/40 p-2.5 rounded-md border border-red-700/70">{error}</p>}
              {message && !loading && <p className="text-sm text-green-400 bg-green-900/40 p-2.5 rounded-md border border-green-700/70">{message}</p>}
              <div>
                ...
                >
                  {isLogin ? 'Initiate Login Sequence' : 'Create Secure Account'}
                </button>
              </div>
            </form>
            <p className="mt-8 text-center text-sm text-gray-400">
              {isLogin ? "No Clearance? " : "Already an Operative? "}
              <Link to={isLogin ? "/signup" : "/login"} className="font-medium text-cyan-400 hover:text-cyan-300 hover:underline">
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-950 py-12 px-4 sm:px-6 lg:px-8 font-['Orbitron',_sans-serif]">
      <div className="absolute inset-0 bg-grid-pattern opacity-[0.03] pointer-events-none animate-pulse"></div> {/* Subtle animated grid */}
