	...
	return (
		...
		>
			...
			<div className="relative z-10 container mx-auto px-4 py-24">
				<div className="max-w-3xl mx-auto bg-gray-900/80 backdrop-blur-md rounded-xl shadow-2xl border border-cyan-700/50 shadow-cyan-500/20 p-10">
					<div className="flex items-center justify-between mb-10">
						<h1 className="text-4xl md:text-5xl font-extrabold text-cyan-400 tracking-wide" style={{ textShadow: '0 0 12px #00fff7, 0 0 20px #00fff7' }}>
							Virtual Contests
						</h1>
						<button
						...
						{contests.map((contest) => (
							...
							>
								<div className="flex flex-col md:flex-row md:items-center md:justify-between">
									<div>
										<h2 className="text-2xl font-bold text-cyan-300 group-hover:text-cyan-400 transition tracking-wide" style={{ textShadow: '0 0 8px #00fff7' }}>
											{contest.name}
										</h2>
									...
									<div className={`mt-4 md:mt-0 md:ml-8 px-4 py-1 border-2 rounded-full font-semibold text-base tracking-wider ${statusColors[contest.status] || "text-gray-400 border-gray-400"}`}>
										{contest.status}
									</div>
								</div>
					...
					{contests.length === 0 && (
						<div className="text-center text-cyan-400 text-xl mt-16">No contests found.</div>
					)}
				</div>
			</div>
		</div>
