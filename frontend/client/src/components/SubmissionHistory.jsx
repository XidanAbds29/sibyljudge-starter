  ...
  if (submissions.length === 0) {
    return (
      <div className="text-center py-10 text-lg">
        ...
        <p className="text-gray-500 text-sm">Submit a solution to see your submission history here.</p>
      </div>
    );
  }
          ...
          >
            ...
            >
              ...
              {selectedSubmission.output && (
                <div className="mt-4">
                  <h4 className="text-lg font-semibold text-yellow-300 mb-2">Output</h4>
                  <div className="bg-gray-800 border border-yellow-700/40 rounded-lg p-4 font-mono text-sm relative">
                    <button
                      onClick={() => navigator.clipboard.writeText(selectedSubmission.output)}
                      className="absolute top-2 right-2 px-2 py-1 text-xs bg-yellow-700/80 hover:bg-yellow-500/80 text-white rounded focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"
                      title="Copy output"
                    >
                      Copy
                    </button>
                    <pre className="text-yellow-200 whitespace-pre-wrap pr-16">{selectedSubmission.output}</pre>
                  </div>
                </div>
              )}
            </motion.div>
...
function NeonGlowSVG({ className = "", style = {}, ...props }) {
  return (
    ...
    >
      <defs>
        <radialGradient id="neon-glow-submissions" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#00fff7" stopOpacity="0.5" />
          <stop offset="100%" stopColor="#00fff7" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx="720" cy="100" rx="700" ry="60" fill="url(#neon-glow-submissions)" />
    </svg>
  );
}
